...
public class ArrayForeach {
	...
	public static void main(String[] args) {
		...
		for(int num : arr2) { 
			// arr2라는 배열의 0번 인덱스부터 순차적으로 접근하면서 int num이라는 변수에 arr2의
			// 각 인덱스의 값을 저장함
		...
		// 사용자에게 구매하고자 하는 폰의 이름을 입력받고 
		// 구매하고자 하는 핸드폰이 phones배열에 있으면 
		// "xx핸드폰의 가격은 xxxxx원 입니다."
		// 구매하고자 하는 핸드폰이 없다면
		// "해당 핸드폰이 없습니다."
		...
		int count = 0;
		for(Phone phone : phones) {
			if(phone.getSeries().equals(p)) {
				System.out.printf("%s %s핸드폰의 가격은 %d원 입니다.",phone.getName(), phone.getSeries(), phone.getPrice());
				count++;
				break;
			}
		}
