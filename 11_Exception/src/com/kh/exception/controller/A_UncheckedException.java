...
public class A_UncheckedException {
	 ...
	 * - NegativeArraySizeException : 배열 크기를 음수로 지정한 경우 발생하는 예외
	 ...
	 * 	  예외 자체가 발생이 안 되게끔 조건문으로 해결이 가능함.
	...
	public void method1() {
		...
		int num2 = sc.nextInt();
		// 해결방법 1. 조건문으로 처리(애초에 예외 자체가 발생되지 않게끔 if문으로 조건검사하기) => 예외처리x
//		if(num2 == 0) {
...
//		System.out.println("프로그램 종료.");
	...
	public void method2() {
		...
		} // 다중 catch블럭으로 사용 가능 : 예외가 여러개 발생할 가능성이 있는 경우
	...
	public void method3() {
		...
		int size = Integer.parseInt(sc.nextLine()); // 형변환메소드 (api wrapper)
		
		
//		if(size > 0) { // 방법 1. if문으로 size값이 양수값일때만 생성하도록 제어.
//			int[] arr = new int[Math.abs(size)]; // 방법 2. 항상 양의 정수값이 들어갈 수 있도록 abs메소드 사용
//		}
...
//					+ "인덱스로 접근한것 같습니다.");
//		} // 다중 catch블럭이 줄어들었음. 정확히 어떤 에러인지는 알 수 없음.
		try {
			int[] arr = new int[size];
			System.out.println("100번 인덱스의 값 : "+arr[100]);
	 ...
	 * - 조건문으로 해결 가능 => 예외 자체가 발생 안 되게끔 개발자가 소스코드로 핸들링 가능.
	 * - 예외처리 구문으로도 해결 가능 => 예외가 발생했을때를 대비해서 그 때 실행할 내용을 정의해두는 것.
	 * 
