package com.kh.chap03_Map.part01_hasMap.run;
...
import com.kh.chap03_Map.part01_hasMap.model.vo.Snack;
...
public class Run {
	...
	public static void main(String[] args) {
		...
		hm.put("다이제", new Snack("초코맛", 1_500));
		hm.put("칸쵸", new Snack("단맛", 900));
		hm.put("포카칩", new Snack("짠맛", 1_000));
		hm.put("포테이토칩", new Snack("짠맛", 1_000));
		
		System.out.println(hm);
		// 1) 저장순서 유지x
		// 2) value값이 동일하다고 해도, key값이 중복되지 않기 때문에 잘 저장됨.
		
		hm.put("포카칩", new Snack("매운맛", 1_200));
		...
		Snack s = hm.get("칸쵸"); // 제네릭설정을 해놨기 때문에 Snack타입 객체로 강제형변환하는 과정이 생략되었다.
		...
		hm.replace("포테이토칩", new Snack("겁나 짠맛", 1_500));
		System.out.println(hm);
		...
		// iterator()는 List와 Set계열에서만 쓰는 메소드이기 때문에 바로 사용 불가.
		// Iterator it = hm.Iterator();
		
		// Map을 Set으로 변경해주는 메소드가 있음. (2개 있음)
