...
public class Run {
	...
	public static void main(String[] args) {
		...
		// 값이 추가될때 equals(), hashCode()로 비교 후 둘 다 결과가 true면 동일객체로 판단.
		...
		// 만약 위 2개의 메소드를 내가 원하는 형태로 바꿔서 중복저장이 안 되게 하고 싶다면 오버라이딩을 해줘야함.
		
		// Student클래스에 equals()오버라이딩 : 세 필드값(name, age, score)이 일치하면 true, 아니면 false
		// Student클래스에 hashCode()오버라이딩 : 세 필드값 기반으로해서 해시코드값을 만들어서 반환
		
		System.out.println("====================================================");
		
		// set => 무작위로 객체가 저장되어있음. (index개념이 없음.) => 반복문활용 불가.
		// 1. for문 사용가능(단, for each문(향상된 반복문)만 가능 => index로 접근이 불가하므로)
