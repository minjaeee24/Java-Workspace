...
public class ListRun {
	...
	public static void main(String[] args) {
		/*
		 * 	컬렉션(Collection)
		 * 	자료구조가 내장되어 있는 클래스로 자바에서 제공하는 "자료구조"를 담장하는 "프레임워크"이다.
		 * 
		 * 
		 * 	자료구조?
		 ...
		 * 
		 * 	정리해보면
		 *	데이터들이 새로이 추가가 되거나, 삭제가 되거나, 수정이 되는 기능(알고리즘)들이 이미 정의되어 있는 틀이 있다
		 * 	== 컬렉션
		 ...
		 * 		=> 한번 지정된 크기는 변경이 불가.
		 * 		   새로운 값을 추가하고자 할때 크기가 오버되는 경우 새로운 크기의 배열을 만들고 기존의 내용들을 복사해주는
		 * 		   코드를 직접 구현해야함.
		 * 	2. 배열 중간 위치에 새로운 데이터를 추가하거나 삭제해야하는 경우 기존의 값들을 땡겨주는 코드를 직접 짜야함.
		 * 	3. 한 타입의 데이터들만 저장 가능.
		 * 
		 * 	- 컬렉션의 장점
		 * 	1. 크기에 제약이 없다.
		 ...
		 * 	3. 기본적으로 여러 타입의 데이터들을 저장할 수 있음(단 제네릭설정을 통해서 한 타입의 데이터들만 들어올 수도 있음)
		 * 
		 ...
		 *  컬렉션의 3가지 분류
		 *  
		 *  List계열 => 담고자하는 값(value)만 저장 / 값 저장시 순서 유지(index의 개념이 있음) / 중복값 허용
		 *  		ex) ArrayList, Vector, LinkedList => ArrayList가 제일 많이 쓰임(90% 이상)
		 *  
		 *  Set계열 => 담고자하는 값(value)만 저장 / 값 저장시 순서 유지 x(index개념이 없음) / 중복값 허용x
		 *  		ex) HashSet, TreeSet
		 *  
		 *  Map계열 => 키(key) + 담고자하는 값(value)세트로 저장 / 값 저장시 순서 유지 x(index개념 없음)
		 *		      / key중복 허용x
		...
		ArrayList<Music> list = new ArrayList<>(/*크기 지정 가능*/); // 내부적으로 크기 10짜리인 배열이 생성됨.
		
		System.out.println(list);
		/*
		 * 	제네릭(<E --> Element>)
		 * 	별도의 제네릭제시 없이 컬렉션객체를 생성하게 되면
		 * 	해당 컬렉션에 다양한타입의 데이터값들이 담길 수 있음. (Default E == Object)
		 * 
		...
		// 3. set(int index, E e) : 리스트에 원하는 위치의 인덱스값을 제시하면 전달하는 e값으로 변경해주는 메소드
		...
		// 제네릭설정 전
//		Music m = (Music) list.get(0); // 다형성을 이용해서 강제형변환.
//		System.out.println(m.getTitle());
		...
		// 9. isEmpty() : 해당 리스트가 비어있는지 묻는 메소드(비어있으면 true / 비어있지 않다면 false 반환)
