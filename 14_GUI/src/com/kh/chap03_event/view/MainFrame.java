...
public class MainFrame extends javax.swing.JFrame {
	...
	public MainFrame() {
		...
		// plusBtn클릭시 => 두 개의 정수값을 가져온 후 덧셈연산 결과를 result텍스트 상자에 출력
		plusBtn.addActionListener(new ActionListener() { // 인터페이스(actionlistener)는 객체생성 불가능하지만 매개변수로는 가능
			@Override
			public void actionPerformed(ActionEvent e) {
				// plusBtn클릭시 실행할 내용
				int value1 = Integer.parseInt(num1.getText());
				int value2 = Integer.parseInt(num2.getText());
				result.setText(value1 + value2 + ""); // int형 문자열로 변경 +""
			}
		});
		// ActionListener : 인터페이스이기때문에 곧바로 생성 불가.
		// 단, 여기서 생성을 하고 싶다면 "미완성된 메소드"를 완성시키면서 생성 => 익명클래스 방식
		
		// minusBtn클릭시 => 두 개의 정수값을 가져온 후 뺄셈연산 결과를 result텍스트상자에 출력
		...
		JLabel title = new JLabel("두 개의 정수를 입력하세요.", 0); // 0 : 가운데정렬
