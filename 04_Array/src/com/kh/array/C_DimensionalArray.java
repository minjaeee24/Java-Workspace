...
public class C_DimensionalArray {
	...
	public void method1() {
		...
		System.out.println(arr); // [[I@69a3d1d
		System.out.println(Arrays.toString(arr[0])); // [I@86be70a
		 ...
		 * 안쪽 for문	=> 열에 대해 지정(0열 ~ 4열) == (0열 ~ 각 행 별 열의크기 - 1)
	...
	public void method4() {
		...
		/*
		 * 가변 배열
		 * 행의 크기는 정해져있으나 각각의 행별로 열의 갯수가 정해져있지 않은 상태
		 * 이차원 배열 == 일차원 배열 여러개를 묶은 형태
		 * 즉, 묶여있는 일차원배열의 길이가 꼭 같을 필요는 없다.
		 * 
		 * 단, 행 크기는 생략이 불가, 반드시 지정해야함.
		 * 열 크기는 생략이 가능하다. 각 행의 열의 길이 변경 가능함.
		 * 
		 */
		
		int[][] arr = new int[3][]; // 가변 배열 (행의 크기 3행)
		...
		// 특정 인덱스로 접근하려했기 때문에 NullPointerException오류가 발생
	...
	public void method6() {
		...
		// 3행 3열짜리 String[][]배열생성 후 
		// 중첩 반복문을 이용해 현재 형, 열의 순번(인덱스)를 출력
		// (0 , 0) (0 , 1)
