...
public class B_ArrayCopy {
	...
	public void method1() {
		...
		//	이유는 얕은복사가 이루어졌기 떄문 => 배열의 주소값이 복사되어
	...
	public void method2() {
		...
		// 1. for문 방법(수작업)
		// 새로운 배열을 생성한 후 각 인덱스별로 내부값을 일일히 대입하는 방법.
	...
	public void method3() {
		...
		//Systam.arraycopy(원본 배열명, 원본배열의 복사를 시작할 인덱스, 복사본 배열명, 복사본 배열의
		// 복사될 시작 인덱스, 복사할 갯수)
	...
	public void method4() {
		...
		// 3. Arrays클래스에서 재공하는 copyOf 메소드를 이용한 복사
		
		int[] origin = {1, 2, 3, 4, 5};
		
		// 복사본 배열 = Arrays.copyOf(원본 배열 명, 복사할 갯수);
		...
		/*
		 * 2. System.arraycopy 메소드 : 몇 번 인덱스부터 몇 개를 어느 위치의 인덱스에
		 * 복사할 것인지 세밀하게 지정 가능.
		 * 
		 * 3. Arrays.copyOf 메도스 : 무조건 원본 배열의 0번 인덱스부터 내가 제시한
		 * 갯수만큼 복사 진행됨. 내가 제시한 길이가 원본 배열보다 크다면 나머지에는 0의 값으로
		 * 채워서 복사해줌.
		 * 
		 */
	}
