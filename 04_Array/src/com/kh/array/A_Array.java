...
public class A_Array {
   ...
   public void method1() {
      ...
      // 각각의 인덱스값을 확인하기 위해서는 [] 블럭을 통해 내가 확인하고자 하는 인덱스(방번호)를 제시해줘야 함
      // 그럼 arr 전체에 담겨있는 값을 확인하고 싶다면?
   ...
   public void method2() {
       ...
       * 5칸짜리 방이 만들어진 후에 가방에 기본값으로 (int 0으로) 데이터가 초기화되고, 주소값이 할당됨 (0x123)
   ...
   public void method3() {
      ...
      /*
       * 내가 각 인덱스를 초기화하지 않았는데도 값들이 담겨있는 이유
       * -> heap이라는 영역에는 절~대로 빈 공간이 존재할 수 없기 때문.
   ...
   public void method4() {
      ...
      for(int i = 0; i < arr.length; i++) {
         arr[i] = i + 1; // arr[0] = 1, arr[1] = 2, arr[3] = 4, arr[4] = 5
      }
   ...
   public void method5() {
      ...
      /*
       * 기본값이 null인 변수를 가지고 메소드를 호출한다던가 특정 어딘가에 접근하고자 한다면
   ...
   public void method6() {
      ...
      for(int i = 0; i < arr.length; i++) {
         System.out.println(arr[i] = (i + 1) * 2);
         //System.out.println("arr[" + i + "] = " + (i+1)*2);
      }
       int value = 2;
       	for(int i = 0; i < arr.length; i++) {
       	 ...
       	 * => 배열이 크기를 경하고자 한다면 어쩔 수 없이 새로운 배열을 다시 만들어야함.
       	 * 
       	 */
    	System.out.println("===== arr 변경후 =====");
       	...
       	System.out.println("arr의 해시코드값 : " + arr.hashCode());
       	/*
       	 * 항상 고유한 주소값 부여, 기존에 생성되었던 주소와 절!대로 겹치지 않는다.
   ...
   public void method7() {
	   ...
	   // 배열 선언 및 동시에 초기화(대입) 까지 한 번에 끝내는 방법 2가지
	   // 방법1.
	   ...
	   // 주소값 == 주소값 : 주소값간의 동등비교를 수행하기때문에 false
   ...
   public void method9() {
   	...
   	public void method10() {
   		...
   		// 2. char배열 생성해두기 (배열 크기 == 문자열의 길이 만큼)
   	...
   	public void method11() {
   		...
   		// 		-> 랜덤값(1~100)
   		for(int i = 0; i < arr.length; i++) { // 0, 1, 2, 3, 4, ... (num-1)
   			arr[i] = (int)(Math.random() * 100 + 1);
   		}
   		
   		// 3. 반복문 활용해서 0번 인덱스부터 마지막 인덱스까지 배열에 담긴 값 출력과 동시에,
   		// 	  해당 인덱스에 담긴 값이 짝수인 경우 그 값들의 총 합 구하기.
   		int sum = 0;
   		for(int i = 0; i < arr.length; i++) {
   			System.out.printf("arr[%d] : %d\n", i, arr[i]);
   			if(arr[i] % 2 == 0) {
   	...
   	public void method12() {
   		...
   		double sum = 0;
   		// 2. 3명의 키에 대한 정보를 출력시키면서 동시에 모든 키에 대한 총 합을 저장
   		for(int index = 0; index < heights.length; index++) {
   			System.out.println(heights[index]);
   			sum += heights[index];
   		}
   		
   		// 3. 3명 키의 총 합을 출력하고, 3명의 평균도 출력 (던, 소숫점 첫째자리까지 출력)
