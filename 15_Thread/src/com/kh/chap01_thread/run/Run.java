...
public class Run {
	 ...
	 * 
	 * 스레드
	 * - 한 개의 프로세스(process)내에서 실제로 작업을 수행하는 소단위
	 ...
	 * 
	 * ex)
	 * - 청소(나, 1간), 빨래(세탁기, 2시간), 설거지(나, 30분), 화장실 청소(나, 30분)
	...
	// 모든 자바 어플리케이션은 MainThread가 main메소드를 실행하면서 시작된다. => 에러코드서도 확인 가능
	public static void main(String[] args) {
		// 메인메소드만 있다면 -> 싱글스레드
		// 항상 앞 라인에 명령문이 다 끝난 후에 실행이 되었음.
		for(int i = 1; i <= 100; i++) {
...
//		// 현재 작업으로 멀티스레드 환경을 만들려면 필요한 스레드의 개수 => 1
		...
		// 스레드 생성방법 2 : Runnable 인터페이스 구현하기
		...
		// 매번 결과가 다르게 보임 => 스케쥴에 따라 순차적으로 번갈아가며 작업이 수행되기 때문.
	}
